...
function normalizeServerAuthMessage(message: string, code = '', hint = '') {
  ...
  return hint ? `${message} ${hint}` : message || code || 'Authentication failed.';
}
...
export async function authenticatedFetch(
  ...
  const responseErrorBody = await readResponseError(response);
  const responseError = normalizeServerAuthMessage(
    responseErrorBody.error,
    responseErrorBody.code,
    responseErrorBody.hint
  );
